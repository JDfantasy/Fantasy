...
-(void)drawCurve:(CGContextRef)context{
    ...
    //绘制曲线
    CGContextMoveToPoint(context, 1, 0);//移动到起始位置
    /*绘制二次贝塞尔曲线
     c:图形上下文
     cpx:控制点x坐标
     cpy:控制点y坐标
     ...
     y:结束点y坐标
     */
    CGContextAddQuadCurveToPoint(context, self.bounds.size.width/4, 0  , self.bounds.size.width/2+5, 2);
    //    CGContextClosePath
    CGContextMoveToPoint(context, self.bounds.size.width/2, 2);
    CGContextAddQuadCurveToPoint(context, self.bounds.size.width/2+9, 30  , self.bounds.size.width/2 +42,128);
    CGContextMoveToPoint(context, self.bounds.size.width/2 +42, 128);
    CGContextAddQuadCurveToPoint(context, 45, 117  , 0, 117);
    CGContextMoveToPoint(context, 0, 117);
    CGContextAddQuadCurveToPoint(context, 120, 50  ,0,1);
    ...
    //    线宽//4.设置图形上下文状态属性
    [UIColor colorWithRed: 0.8041 green: 0.9968 blue: 1 alpha: 1.0];
    CGContextSetRGBStrokeColor(context,0.6, 0.5, 0.8, 1);
    //设置笔触颜色 填充色
